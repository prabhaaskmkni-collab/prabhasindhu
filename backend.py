...
def _check_mx_cached(domain: str, timeout: int = 6):
    return _check_mx(domain, timeout)
...
async def validate_single(request: Request):
    ...
    try:
        ...
        if not has_mx:
            ...
            status = "valid"
            if is_role:
                reason = "Valid — Role-based email address"
            else:
...
async def validate_emails(
    ...
    try:
        ...
        emails = list(dict.fromkeys([e.strip().lower() for e in raw]))[:50000]
        logger.info(f"Validating {len(emails)} emails from {fname} (check_smtp={check_smtp})")

        def _val_single(email):
            syn = _validate_syntax(email)
            if not syn["valid"]:
                return {
            ...
            local, domain = syn["local"], syn["domain"]
            has_mx, _ = _check_mx_cached(domain)
            is_disp = domain in DISPOSABLE_DOMAINS
            is_role = local in ROLE_PREFIXES

            if not has_mx:
                status, reason = "risky", "No MX records found"
            elif is_disp:
                status, reason = "risky", "Disposable email domain"
            else:
                status = "valid"
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            results = list(executor.map(_val_single, emails))

        valid_n = sum(1 for r in results if r["status"] == "valid")
        invalid_n = sum(1 for r in results if r["status"] == "invalid")
        risky_n = sum(1 for r in results if r["status"] == "risky")
        disposable_n = sum(1 for r in results if r.get("is_disposable"))
